...
The very first thing to do when creating a Dead Reckoning Algorithm is to decide exactly what the robot should do. Often, it is easy to sum this up as a short list of commands. Here is the list that we will use for the example program:
...
This is a fundamental step in the creation of your dead reckoning algorithm. The goal of this excercise is to make your dead reckoner match these steps as closely as possible. Unless you already know _exactly_ how fast your robot will be able to accelerate, what it's top speed is, etc. these steps will almost certainly end up being altered later, but for now they will give us our baseline for the program.
...
The [Victor 884](victor-884) receive signals from the [Robot Controller](robot-controller) via [PWM signals](PWM_signal "PWM signal"). These allow the Robot controller to pass values between 0 and 254 to the speed controller. PWM values are stored in 8-bit `unsigned char` so they are limited to values from 0-255, but note that using value of 255 ["resets"](/index.php?title=Resetting_Victors&action=edit "Resetting Victors") the Victor and should not be used.

Also note that on the victor 884 controllers, 0-37 is full reverse, 123-136 is neutral, and 227-254 is full forward.
...
Aliases should probably be placed at the beginning of the `user_routines.h` file. Note that they _should not_ be in `ifi_aliases.c` as it always recommended _not_ to touch the required IFI code.

These lines begin with a hash sign (#). This marks them as [preprocessor](http://www.wikipedia.org/wiki/preprocessor "wikipedia:preprocessor") directives. Note that these lines do not end with a semi-colon like normal C code.
...
The default Autonomous routine is found in `user_routines_fast.c` and looks like this:
...
The basic design of a simple autonomous mode is this: Each step in the code detailed in your functional requirements translates to a step of the program. We will start by working exclusively with the drive train, not any other mechanisms (arms, flippers, etc.) So, our list of directions might be something like this:
...
These are probably best inserted into the default code in user_routines_fast.c file, just after the line

```
/*** DEFINE USER VARIABLES AND INITIALIZE THEM HERE ***/
```

You also need to initialize them every time you go into autonomous. So scroll down to
...
That's it! Now your robot is going forward! There is, of course, one problem. It doesn't stop. At least, not while autonomous mode is on. It's time to use that _timer_ variable we made. The intended result: After a certain time period, go on to the next step. As long as we're at it, we can reset the timer so that it starts out at zero for the next step. With this added, the code for step 0 starts to look like this:
...
Let's spin clockwise! With skid steering, to spin clockwise, or to the right, with a minimum turning radius, the left motor should be directed forward while the right motor should be reversing with the same power. In C...
...
Next, we give this the same treatment that we gave step zero, and it comse out looking like...
...
I leave filling in the remainder of the code as an excercise for the reader. (YOU do steps 2-5)
...
While this is small, it is vital. With out incrementing `timer`, your code would never go anywhere!
...
There's still one thing fundamentally wrong with our autonomous code. So far, for the timer values, we've just thrown in the value 400\. This is, in almost absolute certainty, not going to make the robot go forward anything near 10 feet or spin clockwise 45 degrees.

It's time for an application of the classic problem solving technique of _Guess & Check_. Put any values that you think are reasonable into place instead of those 400s. Download the code into your robot, set it into autonomous mode with a [Competition port dongle](Competition_port_dongle "Competition port dongle"), and see what it does. If it goes too far, take some cycles off the appropriate timer. If it doesn't spin enough, on that spin timer add a few extra cycles. The idea here is to just play with it until you get it right!
...
The dead reckoning algorithm described in this article has a few severe limitations.
